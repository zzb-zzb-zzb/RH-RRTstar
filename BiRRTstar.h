    ...
    namespace geometric
    {
        ...
        public:
            ...
            */
            ...
            void setrevCollisionChecker(std::shared_ptr<CollisionChecker> cck)
            {
                collision_checker_ = cck;
            }

            void setrevstepsize(double size)
            {
                rev_stepsize=size;
            }

            void setheuristicBias(double bias)
            {
                heuristicBias_=bias;
                init_heuristicBias_=bias;
            }

            void setrmeet(double r)
            {
                r_meet_=r;
            }
            ...
            void setrevcontinue(bool rc)
            {
                rev_continue=rc;
            }

            std::vector<std::pair<double,double>> getheuristicData();
        ...
        protected:
            ...
            void rev_getNeighbors(Motion *motion, std::vector<Motion *> &nbh) const;
            ...
            base::StateSamplerPtr rev_sampler_;
            ...
            std::shared_ptr<NearestNeighbors<Motion *>> rev_nn_;

            std::shared_ptr<base::RealVectorStateSpace> space_;
            std::shared_ptr<geometric::SimpleSetup> ss_;
            base::SpaceInformationPtr rev_si_;
            base::StateSpace *rev_stateSpace_;

            std::vector<Motion*> heuristic_path;
            std::vector<int> heuristic_idx;

            std::shared_ptr<CollisionChecker> collision_checker_;
            double heuristicBias_{0.2};
            double init_heuristicBias_{0.2};
            double rev_stepsize{1};
            double r_meet_{10.0};
            double n_{3};
            bool rev_continue=true;
            double alpha{1.5};

            double rev_distanceFunction(const Motion *a, const Motion *b) const
            {
                return rev_si_->distance(a->state, b->state);
            }

            bool revcheckMotion(const base::State *s1, const base::State *s2);

            void convertRealVectorToSE2(const Motion* rev_motion, base::State* se2_state, bool disturb);
            ...
            base::OptimizationObjectivePtr rev_opt_;

            /** \brief The best goal motion. */
            Motion *bestGoalMotion_{nullptr};
            Motion *rev_bestGoalMotion_{nullptr};

            /** \brief A list of states in the tree that satisfy the goal condition */
            std::vector<Motion *> goalMotions_;
            std::vector<Motion *> rev_goalMotions_;
            ...
            base::Cost rev_bestCost_{std::numeric_limits<double>::quiet_NaN()};
            ...
            std::pair<double, double> random_point_in_ellipse(double a, double b)
            {
